...
import {
  Search, Zap, MessageCircle, ArrowRight, CheckCircle2,
  TrendingUp, Globe, Bot, Wrench, Star, Sparkles, Award, Trophy,
} from 'lucide-react';
...
const WHATSAPP_URL =
  'https://wa.me/17865787671?text=Hola%20Ferova%2C%20quiero%20una%20propuesta%20para%20mi%20e-commerce';

type Copy = {
  seoTitle: string;
  seoDesc: string;
  // Hero
  h1a: string; h1b: string; heroSub: string;
  badges: string[];
  ...
  // Solución
  solutionTitle: string; solutionSub: string;
  services: { icon: any; title: string; desc: string; href: string }[];
  // Resultados
  resultsTitle: string; resultsSub: string;
  metrics: { value: string; label: string }[];
  testimonialName: string; testimonialRole: string; testimonialText: string;
  // TAYA
  tayaTitle: string; tayaSub: string;
  faqs: { q: string; a: string }[];
  // CTA final
  finalTitle: string; finalSub: string;
  finalCtaWa: string; finalCtaAi: string;
};
...
const COPY: Record<'es' | 'en' | 'pt', Copy> = {
  es: {
    ...
    problems: [
      { icon: Search, title: 'No te encuentran orgánicamente', desc: 'Tu marca no aparece en Google ni en las respuestas de IA. Pagas anuncios para sobrevivir, no para crecer.' },
      { icon: Zap, title: 'Web lenta que no convierte', desc: 'Tu sitio carga en 5+ segundos, falla en mobile y pierdes 53% de visitantes antes del primer scroll.' },
      { icon: MessageCircle, title: 'Atención al cliente saturada', desc: 'Tu equipo responde las mismas preguntas todo el día por WhatsApp y los leads calientes se enfrían.' },
    ],
    ...
    services: [
      { icon: Globe, title: 'Web Apps E-commerce', desc: 'Tiendas ultrarrápidas (<1s), seguras y diseñadas para convertir desde el primer clic.', href: '/servicios/diseno-web' },
      { icon: Search, title: 'SEO + GEO + IAO', desc: 'Posicionamiento orgánico, geográfico y para motores de IA. Tu marca citada por ChatGPT, Perplexity y Google.', href: '/servicios/seo-ecommerce' },
      { icon: Bot, title: 'WhatsApp IA Bot', desc: 'Asistente que responde, califica y cierra ventas 24/7 en WhatsApp Business con tu tono de marca.', href: '/servicios/whatsapp-ia-bot' },
      { icon: Wrench, title: 'Herramientas Premium', desc: 'Acceso a herramientas de SEO, automatización e IA con descuentos exclusivos para clientes.', href: '/servicios/descuentos-herramientas' },
    ],
    ...
    finalSub: 'Habla con nuestro equipo por WhatsApp o prueba a Fera, nuestra Asesora IA, para diagnosticar tu caso en 2 minutos.',
  ...
  en: {
    ...
    problems: [
      { icon: Search, title: 'You can\'t be found organically', desc: 'Your brand doesn\'t show up on Google or in AI answers. You pay ads to survive, not to grow.' },
      { icon: Zap, title: 'A slow site that doesn\'t convert', desc: 'Your site loads in 5+ seconds, breaks on mobile and you lose 53% of visitors before the first scroll.' },
      { icon: MessageCircle, title: 'Customer support overloaded', desc: 'Your team answers the same questions all day on WhatsApp and hot leads go cold.' },
    ],
    ...
    services: [
      { icon: Globe, title: 'E-commerce Web Apps', desc: 'Ultra-fast (<1s) stores, secure and designed to convert from the first click.', href: '/en/services/web-design' },
      { icon: Search, title: 'SEO + GEO + AIO', desc: 'Organic, geographic and AI-engine ranking. Get cited by ChatGPT, Perplexity and Google.', href: '/en/services/ecommerce-seo' },
      { icon: Bot, title: 'WhatsApp AI Bot', desc: 'Assistant that replies, qualifies and closes sales 24/7 on WhatsApp Business in your brand voice.', href: '/services/whatsapp-ai-bot' },
      { icon: Wrench, title: 'Premium Tools', desc: 'Access to SEO, automation and AI tools with exclusive client discounts.', href: '/en/services/tool-discounts' },
    ],
  ...
  pt: {
    ...
    problems: [
      { icon: Search, title: 'Você não é encontrado organicamente', desc: 'Sua marca não aparece no Google nem nas respostas de IA. Você paga anúncios para sobreviver, não para crescer.' },
      { icon: Zap, title: 'Site lento que não converte', desc: 'Seu site carrega em 5+ segundos, falha no mobile e você perde 53% dos visitantes antes do primeiro scroll.' },
      { icon: MessageCircle, title: 'Atendimento sobrecarregado', desc: 'Seu time responde as mesmas perguntas o dia todo no WhatsApp e os leads quentes esfriam.' },
    ],
    ...
    services: [
      { icon: Globe, title: 'Web Apps E-commerce', desc: 'Lojas virtuais ultrarrápidas (<1s), seguras e desenhadas para converter desde o primeiro clique.', href: '/pt/design-web' },
      { icon: Search, title: 'SEO + GEO + IAO', desc: 'Posicionamento orgânico, geográfico e para motores de IA. Sua marca citada por ChatGPT, Perplexity e Google.', href: '/pt/seo-ecommerce' },
      { icon: Bot, title: 'WhatsApp IA Bot', desc: 'Assistente que responde, qualifica e fecha vendas 24/7 no WhatsApp Business com a voz da sua marca.', href: '/pt/whatsapp-ia-bot' },
      { icon: Wrench, title: 'Ferramentas Premium', desc: 'Acesso a ferramentas de SEO, automação e IA com descontos exclusivos para clientes.', href: '/pt/ferramentas' },
    ],
    ...
    finalSub: 'Fale com nossa equipe no WhatsApp ou teste a Fera, nossa Consultora IA, para diagnosticar seu caso em 2 minutos.',
...
const Index = ({ lang = 'es' }: IndexProps) => {
  ...
  const openAiAdvisor = () => {
    ...
    const trigger = document.querySelector<HTMLButtonElement>(
      'button[aria-label*="IA"], button[aria-label*="AI"]'
    );
    trigger?.click();
  };
      ...
      <main className="flex-1">
        ...
        >
          ...
          <div className="relative max-w-6xl mx-auto px-4 sm:px-6 text-center">
            ...
            <p className="text-base md:text-xl text-white/75 max-w-3xl mx-auto leading-relaxed mb-8">
              {c.heroSub}
            </p>
        ...
        {/* ===== SECCIÓN 3 — SOLUCIÓN ===== */}
        <section
          className="py-16 md:py-24 px-4 sm:px-6 dark-section"
          style={{ background: 'hsl(243 31% 10%)' }}
        >
          <div className="max-w-6xl mx-auto">
            <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
            ...
            <Accordion type="single" collapsible className="w-full space-y-3">
              {c.faqs.map((f, i) => (
                <AccordionItem
                  key={i}
                  value={`item-${i}`}
                  className="glass-card border px-5 md:px-6 !border-border"
                >
        ...
        <section
          ...
          style={{
            background:
              'linear-gradient(135deg, hsl(243,31%,10%) 0%, hsl(356,68%,20%) 100%)',
          }}
        >
