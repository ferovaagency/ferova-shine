...
const Contacto = ({ lang = 'es' }: ContactoProps) => {
  ...
  const inputCls = "w-full px-4 py-3 rounded-xl text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-neon-cyan/50 transition-all border" +
    ...
    <>
      ...
      <main className="pt-20">
        <section className="py-20 md:py-28 relative grid-pattern">
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 30%, hsla(251, 100%, 69%, 0.06), transparent 60%)' }} />
          <div className="container mx-auto px-4 md:px-6 relative z-10">
            ...
            <div className="grid lg:grid-cols-5 gap-10 max-w-6xl mx-auto">
              ...
              <form onSubmit={handleSubmit} className="lg:col-span-3 glass-card p-8 md:p-10 space-y-5">
                ...
                <label className="flex items-start gap-3 text-sm text-muted-foreground cursor-pointer">
                  <input
                    ...
                    className="mt-1 rounded border-border accent-neon-cyan"
                  />
                  <span>
                    {t.consent}{' '}
                    <a href={lang === 'es' ? '/privacidad' : '/en/privacy'} className="text-neon-cyan underline underline-offset-2">
                      {t.consentLink}
                    </a>.
                  </span>
                </label>
                <button
                  type="submit"
                  disabled={sending}
                  className="btn-neon w-full !py-4 flex items-center justify-center gap-2 disabled:opacity-60"
                >
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <a href="https://wa.me/17865787671" target="_blank" rel="noopener noreferrer"
                  className="glass-card p-5 flex items-center gap-4 hover:border-neon-cyan/30 transition-all group block">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'hsla(142, 70%, 45%, 0.1)' }}>
                    <MessageCircle className="w-6 h-6" style={{ color: '#25D366' }} />
                  </div>
                  <div>
                    <p className="font-display font-semibold text-foreground">{t.whatsapp}</p>
                    <p className="text-muted-foreground text-sm">{t.whatsappDesc}</p>
                  </div>
                </a>

                <a href="mailto:info@ferova.agency"
                  className="glass-card p-5 flex items-center gap-4 hover:border-neon-cyan/30 transition-all group block">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'hsla(187, 100%, 50%, 0.1)' }}>
                    <Mail className="w-6 h-6 text-neon-cyan" />
                  </div>
                  <div>
                    <p className="font-display font-semibold text-foreground">{t.emailTitle}</p>
                    <p className="text-muted-foreground text-sm">{t.emailDesc}</p>
                  </div>
                </a>

                <a href="https://calendly.com/tu-enlace" target="_blank" rel="noopener noreferrer"
                  className="glass-card p-5 flex items-center gap-4 hover:border-neon-cyan/30 transition-all group block">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'hsla(251, 100%, 69%, 0.1)' }}>
                    <Calendar className="w-6 h-6 text-neon-purple" />
                  </div>
                  <div>
                    <p className="font-display font-semibold text-foreground">{t.schedule}</p>
                ...
                <div className="glass-card p-5 flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-display font-semibold text-foreground">{t.location}</p>
                  </div>
                </div>
              </div>
            </div>
